...
{
    if (input->last != input->first)				// if length(list) >= 1
    {
        list_element * pivot = input->first;		
        list_element * head = input->first->next;
        list_element * tmp = head;
        while(head != NULL)							// loop over list
        {
            tmp = head;
            head = head->next;
            tmp->next = NULL;
            if (tmp->count < pivot->count)			// if(current_item < pivot) left.append(current_item)
            {
                if (left->last == NULL)
                {
                    left->first = tmp;
            ...
            } else {								//else right.append(current_item)
                if (right->last == NULL)
                {
                    right->first = tmp;
                    right->last = tmp;
